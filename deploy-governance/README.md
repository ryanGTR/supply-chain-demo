...
| `promote.py` | **build-once 多環境 promote**:同一 artifact test→uat→prod 逐區搬移 + 每區重新驗章;順序/禁跳關、正式區需 CAB 核可(fail-closed) |
| `validate_change_class.py` | **變更分類閘門**(T2.4,fail-closed):急件/插單/補單規則 + ★繞過旗標守衛(簽章/掃描/驗章不可因 changeType 關閉) |
| `reconcile_release.py` | **發佈漂移對帳**(T2.4):CMDB 期望態 vs 發佈庫實際態(digest 不一致/庫內被掉包/記錄聲稱發佈但庫裡沒有 → fail-closed) |
| `tests/selftest.sh` | L4 驗章:1 正向 + 6 負向(未簽/竄改/缺證據/掃描 fail/空測試/壞 digest)+ CMDB |
| `tests/promote-selftest.sh` | promote:逐區晉級 / 禁跳關 / 正式區核可 / 來源竄改 / build-once(6 案例) |
| `tests/change-governance-selftest.sh` | 變更治理:變更分類 10 案 + 漂移對帳 3 案(正向 + 負向,fail-closed) |
