...
- ✅ Tier 2 進行中：**Vault transit 簽章後端**(`mode: hashivault`,私鑰留 Vault;PoC 在 `vault-research/demo/transit-cosign/`)+ **build-once 多環境 promote**(`promote.py`,test→uat→prod 逐區重驗 + 正式區 CAB 核可)。
- ⏳ 後續：變更治理深化、前端/dotnet 檔案型路線、上 ADO 公司版。
